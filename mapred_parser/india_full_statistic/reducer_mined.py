...
class MergeItem:
  ...
  def reset(self, user_url=None):
    ...
    self._is_india_country = False
    self._is_india_mined_country = False
    self._url = None
  ...
  def add_item(self, user_url, data_type, url, data_base64):
    if data_type == 'user':
      try:
        ...
        if self._user:
          if self._user.country == 'IN':
            self._is_india_country = True
          if self._user.mined_countrys and 'IN' in self._user.mined_countrys:
            self._is_india_mined_country = True
          if self._is_india_country or self._is_india_mined_country:
      ...
      if url != self._url:
        ...
        if self._is_india_user:
          ...
          if self._is_india_country:
            sys.stderr.write('reporter:counter:statistic,india_country_total,1\n')
          if self._is_india_mined_country:
            sys.stderr.write('reporter:counter:statistic,india_mined_country,1\n')
          if not self._is_india_country and self._is_india_mined_country:
            sys.stderr.write('reporter:counter:statistic,only_mined_country,1\n')
