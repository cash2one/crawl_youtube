...
from ..base.thrift_util import SERIALIZE_TYPE_FAST
...
def get_category_name_id(shortname):
  if not shortname:
    return None
  if 'movie' == shortname:
    return '电影', 1
  ...
  if 'sport' == shortname:
    return '体育', 4
  if 'animation' == shortname:
    return '动漫', 5
  if 'finance' == shortname:
    return '财经', 22
  if 'joke' == shortname:
    return '搞笑', 10
  if 'fashion' == shortname:
    return '风尚', 20
  else:
    return '资讯', 6
...
class RealtimeVideoWriter(PageWriterWithBuff):
  ...
  def _get_channel(self):
    for ip in self.ips_:
      try:
        self.rabitt_ = pika.BlockingConnection(pika.ConnectionParameters(host=ip, port=self.port_))
  ...
  def get_connect_mysql(self):
    while not self.exit_:
      try:
        ...
        self.spider_.log('Error connect to mysqldb[%s]:[%s, %s, %s, %s]' %
                         (e, self.mysql_host_, self.mysql_port_,
                          self.mysql_db_, self.mysql_usr_), log.ERROR)
  ...
  def _batch_fill_items(self, item):
    if not item:
      return None
    retlist = []
    status, links = self.new_links_extract.extract_block_links(item['url'],
                                                               body=item['page'],
                                                               bd_type=LinksExtractor.HTML_EXTRA)
    for i in links:
      if not i.extend_map:
        continue
      ...
      if video_m:
        ...
        self.spider.log("Failed convert item to MediaVideo:%s" % item,
                        log.ERROR)
    return retlist


  def _fill_category_id(self, url):
    if not self.using_links_extract_:
      catid = self.extend_map_handler_.settings.get_category_name(url=url)
      catnid = None
      if catid:
        catnid = get_category_name_id(catid)
      return catnid if catnid else None
    else:
  ...
  def _fill_video_media_from_extend_map(self, extend_map, url, ref_url):
    ...
    if sourid:
      ...
      return None
    # extend map
    catnid = self._fill_category_id(url) or self._fill_category_id(ref_url)
    if catnid:
      media_video.category, media_video.category_id = catnid[0], catnid[1]
    else:
      self.spider.log('Failed Found cateid, %s, %s' % (url, ref_url), log.ERROR)
      return None
  ...
  def _mysql_writer(self, redict):
    try:
      ...
      if cursor.execute("""select id from %s where id='%s'""" % (self.tbl_n, idstr)) > 0:
        ...
        self.release_connect_mysql()
        return
      cursor.execute("""insert into %s (id, base64_thrift) values (%%s, %%s)""" %
                     self.tbl_n, (idstr, redict['object']))
      conn.commit()
