...
from utils import str_gzip
from url_normalize import UrlNormalize
from start_url_loads import StartUrlsLoader
...
class TodayTvWriter(PageWriterWithBuff):
  def __init__(self, spider):
    ...
    if self.debug_: # local test
      ...
      self.mysql_host_ = '10.176.28.126'
      self.mysql_port_ = 3306
  ...
  def __write_mysql_internal(self, item):
    ...
    try:
      ...
      tmp_ext = '{}'
      location_str = \
          '%s' % self.extend_map_handler_.settings.get_location_from_referer(refer_url) or ''
      tmp_ext = self.extend_map_handler_.lookup_extend_map(url)
      if not tmp_ext:
        for u in item.get('redirect_urls', []):
          tmp_ext = self.extend_map_handler_.lookup_extend_map(u)
          if tmp_ext: break
        if not tmp_ext:
      ...
      if ac_category:
        category = ac_category.split('_')[0]
      else:
        category = ''
