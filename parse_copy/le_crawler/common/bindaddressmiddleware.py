...
class BindaddressMiddleWare(object):
  def __init__(self, settings):
    if not settings['BINDIP_LOCAL']:
      ...
      self._ips = self._get_ips_auto('eth1') or self._get_ips_auto('eth0')
  ...
  def _get_ips_auto(self, eth):
    cmd = "ifconfig | grep -A 3 %s | grep 'inet addr' | awk -F ' ' '{print $2}' | awk -F ':' '{print $2}'" % eth
