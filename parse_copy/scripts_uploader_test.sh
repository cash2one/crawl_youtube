...
#cd ../mapred_parser && \
#echo 'backing up binary...' && hadoop fs -mv /user/search/short_video/bin/user_test/${binary} /user/search/short_video/bin/user_test/${binary}_$(date +%Y%m%d_%H%M%S)
#echo 'uploading...' && hadoop fs -put ${binary} /user/search/short_video/bin/user_test/ && \
#rm -f ${binary} && \
echo 'Completed.'
