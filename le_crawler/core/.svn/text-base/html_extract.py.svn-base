...
from base_extract import ExtractLinks
...
class HtmlExtractor(BaseExtractor):
  ...
  def __get_xpath_value(self, cur_sel, reglist, extract=True):
    ...
    for regtmp in reglist[2]:
      ...
      if regrst:
        return regrst[regindex] if 0 <= regindex < len(regrst) else ''.join(regrst)
    return None
  ...
  def _extract_extend_map(self, body, id=None, pageurl=None, bd_type='html', filter_url=True):
    returls = []
    tmpid = id
    ...
    try:
      for urls in rootsels:
        ...
        if not url:
          ...
          continue
        if filter_url and not self.accept_url(url):
          self.loger.log.info('not accepted url: [%s]' % url)
          continue
        url = url.strip()
  ...
  def _extract_custom_map(self, body, localid=None, pageurl=None, bd_type='html'):
  ...
  def get_category_name(self, url):
    return self.start_url_loader.get_property(url, 'category', None) if self.start_url_loader else None
