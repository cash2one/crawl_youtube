...
class CdPicturesAlbumExtractor(object):
  def __init__(self, spider, *kargs, **kwargs):
    ...
    self.qq_js_post = ['hdBigPic.js', 'hdBigPic.js', 'hdPic.js', 'hdBigPic.js', 'hdPic.js']
    import re
    self.qq_aritcle_time_reg = \
    re.compile(r'\.qq\.com\/.*\/(\d+)\/\d+.htm')
    self.links_extractor =\
        ExtraExtendMapEngine(kwargs['start_url_loader'],
            'le_crawler.common.cdesktop_settings', None)

  # return album_request_urls
  def _parser_sina_album_list(self, url, jsobj):
    albumurls = []
    for alb in jsobj:
      ch = alb.get('ch')
      aid = alb.get('aid')
      ...
      if not ch or not aid or not sid or not album_url:
        self.spider.log('Bad Album Info,%s %s' % (url, alb), log.ERROR)
        continue
      request_url =\
  ...
  def _parser_sohu_album_info(self, raw_url, jsobj):
    ...
    for alb in albums:
      ...
      for i in imglistobj:
        ...
        if tmpimg:
          imgs.append(tmpimg)
      if not urlid:
        self.spider.log('Bad Album Info:%s, %s' % (raw_url, alb), log.ERROR)
        continue
  ...
  def _parser_qq_album_list_page(self, body, url):
    ...
    for url in urlexts:
      ...
      if times > '20140101':
        article_time_raw = tmppropery.get('article_time_raw', article_time_raw)
      if article_time_raw:
        article_time_raw = article_time_raw.replace(' ', ' ')
        self.album_infos.setdefault(url, {})['article_time_raw']\
            =  article_time_raw
      if 'title' in tmppropery:
        self.album_infos.setdefault(url, {})['title'] = tmppropery['title']
  ...
  def _parser_qq_album_info(self, body, url, content_type):
    if 'javascript' not in content_type or 'application' not in content_type:
      self.spider.log('Bad qq js page content:type(%s), %s'
          % (content_type, body),
          log.ERROR)
      return {}
    tmpdict = {}
  ...
  def parser_album_list_page(self, url, body):
    import json
    if 'sina.cn' in url:
      return self._parser_sina_album_list(url, json.loads(body))
    elif 'm.sohu.com' in url:
      return self._parser_sohu_album_info(url, json.loads(body))
    elif 'qq.com' in url:
