  ...
  def _netease_exetend_map(self, sel, enc='gb2312'):
    ...
    try:
      ...
      for urls in aurls:
        ...
        tmptitle = urls.xpath('.//img/@alt').extract() or urls.xpath('./text()').extract() or urls.xpath(
          './/div[@class="text"]/h3/text()').extract()
    ...
    try:
      ...
      for urls in all_urls:
        ...
        if not self._is_intresting_url(storeurl):
          continue
        tmppic = urls.xpath('./img/@src').extract()
        tmptitle = urls.xpath('./@title').extract() or urls.xpath('./@alt').extract() or urls.xpath(
          './img/@alt').extract() or urls.xpath('./img/@title').extract() or urls.xpath('./text()').extract()
        extend_s = self._gen_store_result(encoding=enc, lurl=tmpurl, lpic=
        tmppic, ltitle=tmptitle)
        if extend_s:
    ...
    try:
      ...
      for urls in alls:
        ...
        if not self._is_intresting_url(store_url):
          continue
        tmppic = urls.xpath('./img/@src').extract()
        tmptitle = urls.xpath('./img/@alt').extract() or urls.xpath('./@title').extract() or urls.xpath(
          './text()').extract()
        extend_s = self._gen_store_result(encoding=enc, lurl=tmpurl, lpic=
        tmppic, ltitle=tmptitle)
        if extend_s:
