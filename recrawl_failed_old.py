  ...
  def run(self):
    while not self.exit_:
      ...
      #for item in self._collection.find({}):
        ...
        update_item = UpdateOne({'url': item['url']},{'$set': {'next_schedule_time': time_now + 10800,'status': CrawlStatus._VALUES_TO_NAMES.get(CrawlStatus.SCHEDULING), 'retry_times': retry_times + 1}}, upsert=True)
        update_items.append(update_item)
        self.logger_.info('recrawl url: %s', item['url'])
        if len(docs) >= 50:
