...
class PageWriterWithBuff(PageWriterBase):
  ...
  def finalize(self):
    ...
    while not self.wthread_exit_:
      ...
      time.sleep(3)
    self.spider_.log('[%s] total writer item %s' %
        (self.name, self.__write_count), log.INFO)

  def status(self):
    return str('[%s]\'s queue size#: %d, has processed#: %d' %
        (self.name,
          self.data_queue_.qsize(),
          self.__write_count))
  ...
  def _writer_thread(self):
    ...
    while not self.exit_sig or not self.data_queue_.empty():
      ...
      try:
        ...
        import traceback
        error_msg = e.message + traceback.format_exc()
        self.spider_.log('write manager exit with exception:%s' % error_msg)
