...
from le_crawler.base.singleton import Singleton
...
class ExtendMapHandler(object):
  ...
  @staticmethod
  def get_instance(module_path = 'le_crawler.common.headline_video_settings',
      start_urls_path = '../start_urls/'):
    ExtendMapHandler._instance_lock.acquire()
    if not ExtendMapHandler._instance:
      ExtendMapHandler._instance = ExtendMapHandler(module_path, start_urls_path)
  ...
  def __init__(self,
    ...
    self.setting_handler_ = ExtraExtendMapEngine(
        ...
        start_urls_path = start_urls_path,
        loger = self.loger)
