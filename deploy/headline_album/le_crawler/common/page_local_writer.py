...
class PageLocalWriter(PageWriterBase):
  ...
  def __init__(self, spider):
    ...
    if get_project_settings()['DEBUG_MODEL']:
      self._init(86400, 200, '/tmp/crawler_delta/')
      self.set_name('PageLocalWriterTest')
    else:
      self._init(86400, 20000, '/letv/crawler_delta/')
      self.set_name('PageLocalWriter')
  ...
  def gen_filestr(self):
    return os.path.join(self.data_dir_, '%s_%d'%(time.strftime('%Y%m%d_%H%M%S', time.localtime()),
        os.getpid()))
  ...
  def _dump_file(self):
    try:
      ...
      self.last_flush_time_ = int(time.time())
      self.file_fp_ = None
      os.rename(self.current_file_name_ + '.tmp', self.current_file_name_)
      return True
    except Exception, e:
      print e
  ...
  def file_writer_manger(self):
    while not self.exit_:
      ...
      # tell the real time status
      #if nows % 10 == 0:
        #num_per_s = self.current_nums_ / (nows - self.last_flush_time_ + 1)
        #msg = 'real time speed:[%d, %d, %d]' % (num_per_s, self.current_nums_, self.total_items_)
        #self.spider_.log(msg, log.INFO)

    self.spider_.log('file write manager exit normal', log.INFO)
    self._dump_file()
