...
class TodayTvWriter(PageWriterWithBuff):
  def __init__(self, spider):
    ...
    if self.debug_: # local test
      ...
      self.mysql_host_ = '127.0.0.1'
      self.mysql_port_ = 3306
      self.mysql_usr_ = 'headline_wr'
  ...
  def __write_mysql_internal(self, item):
    ...
    try:
      ...
      mapid = self.extend_map_handler_.settings.get_id_from_url(refer_url)
      if not mapid:
      ...
      if cursor.execute("""select id from %s where id='%s'""" % (tbl_n, idstr)) > 0:
        ...
        if self.extend_map_handler_.settings.delete_dupe_data(id = mapid):
          if cursor.execute("""delete from %s where id='%s'""" % (tbl_n, idstr)) > 0:
            self.spider_.log('delete data from %s, where id=%s' % (tbl_n, idstr),
               log.INFO)
          else:
            self.spider_.log('Failed delete data from %s, where id=%s' % (tbl_n, idstr),
               log.ERROR)
        else:
          return
      ...
      cpage = str_gzip(page)
      tmp_ext = '{}'
      if not self.extend_map_handler_.settings.ignore_id(mapid):
        tmp_ext = self.extend_map_handler_.lookup_extend_map(url)
        if not tmp_ext:
          tmp_ext = '{}'
          self.spider_.log('Failed lookup extend map for %s' % (url), log.ERROR)
      ac_category = self.extend_map_handler_.settings.get_category_name(id = mapid)
      cursor.execute("""insert into %s 
      (id, url, parse_state, category, fetch_time, extend_map, html) values
      (%%s, %%s, %%s, %%s, %%s, %%s, %%s)""" % (tbl_n), (idstr, url, 0, ac_category,
        int(time.time()) * 1000, str(tmp_ext), cpage))
      conn.commit()
      #cursor.close()
      self.store_pages_ += 1
