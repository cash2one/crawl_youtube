...
class TimerDispatcher(object):
  ...
  def send_singal(self, send_singal, next_timeout, idx):
    self.crawler.signals.send_catch_log(signal=send_singal)
    if idx >= 0:
  ...
  def start_timer(self):
    self.send_singal(le_crawler.signals.minutely_timeout, 60, -1)
    self.send_singal(le_crawler.signals.hourly_timeout, 60*60, -1)
    print 'send'
    self.send_singal(le_crawler.signals.minutely_timeout, 60*60*24, -1)
    for k, v in self.crawler.settings.get("CUSTOM_TIMERS", {}).items():
      sig = getattr(le_crawler.signals, k, None)
      if sig:
